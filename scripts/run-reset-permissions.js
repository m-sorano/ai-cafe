...
// 直接テストファイルをアップロードしてみる
async function testUpload() {
  try {
    console.log('テストファイルのアップロードを試みます...');
    ...
    const { data, error } = await supabase.storage
      .from('avatars')
      .upload('test-upload.txt', fs.createReadStream(testFilePath), {
        cacheControl: '3600',
        upsert: true
      });
    ...
    if (error) {
      console.error('テストアップロードに失敗しました:', error);
      return false;
    }
    
    console.log('テストアップロードに成功しました:', data);
    return true;
  } catch (error) {
    console.error('テストアップロード中にエラーが発生しました:', error);
    return false;
  }
}

async function main() {
  console.log('=== Supabase RLSポリシーリセットスクリプト ===');
  ...
  // ストレージのRLSポリシーをリセット
  const storageResult = await runSqlFile('./docs/reset-storage-permissions.sql');
  
  if (storageResult) {
    console.log('✅ ストレージのRLSポリシーをリセットしました');
  } else {
    console.error('❌ ストレージのRLSポリシーのリセットに失敗しました');
  }
  
  // プロフィールテーブルのRLSポリシーをリセット
  const profilesResult = await runSqlFile('./docs/reset-profiles-permissions.sql');
  
  if (profilesResult) {
    console.log('✅ プロフィールテーブルのRLSポリシーをリセットしました');
  } else {
    console.error('❌ プロフィールテーブルのRLSポリシーのリセットに失敗しました');
  }
  ...
  if (testResult) {
    console.log('✅ テストアップロードに成功しました');
  } else {
    console.error('❌ テストアップロードに失敗しました');
  }
  
  if (bucketResult && storageResult && profilesResult && testResult) {
    console.log('\n✅✅✅ すべての修正が正常に完了しました');
